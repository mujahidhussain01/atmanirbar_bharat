...
class Settings extends CI_Controller
{
    ...
    public function View_loan_setting()
    {
        $this->data['page'] = 'loan_setting';
        $this->data['data'] = $this->loan_setting_model->get_loan_settings();
        $this->load->view('admin/loan_setting', $this->data);
    }
    public function loan_setting()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $data['data'] = $this->loan_setting_model->get_loan_settings();
            $this->load->view('admin/loan_setting', $data);
        } else {
            ...
            redirect('admin/Settings/loan_setting');
        }
    }
    public function view_edit_modal($lsid)
    {
        $result = $this->loan_setting_model->getloan_setting($lsid); ?>
        <form data-action="<?php echo base_url() ?>admin/settings/update_loan_setting/<?= $result['lsid'] ?>" onsubmit="return edit_loan(this)" method="post" class="margin-bottom-0">
            <div class="form-row">
                <div class="col-md-6 form-group m-b-15 ">
                    <label>Loan Name:</label>
                    <input type="text" minlength="2" maxlength="100" class="form-control form-control-md" placeholder="Loan Name" value='<?php echo $result['loan_name'] ?>' required name="loan_name" id='loan_name'>
                </div>
                <div class="col-md-6 form-group m-b-15 ">
                    <label>Amount:</label>
...
<?php
    ...
    {
        $this->data['page'] = 'user_otp';
        $this->data['otps'] = $this->Otp_model->getAllOtpdata();
        $this->load->view('admin/user_otp', $this->data);
    }
    public function my_bank_details()
    {
        $this->data['page'] = 'my-bank-details';
        $this->data['admin'] = $this->Adminlogin->get_admin();
        $this->load->view('admin/my_bank_details', $this->data);
    }
    public function save_my_bank_details($id)
    {
        $result = $this->Adminlogin->update($id, $_POST);
        if ($result > 0) {
            $this->session->set_flashdata('success', 'Your Bank Details updated Succsesfully successfully!');
        } else {
            $this->session->set_flashdata('error', 'Error in updating bank details!');
        }
        redirect('admin/Settings/my-bank-details');
    }
}
