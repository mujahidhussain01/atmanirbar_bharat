...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".panel-body").on("change keyup", "input[type='number'],select", function() 
		{
			...
			var emi_count_show = $( parentForm ).find('.emi_count_show');
			var loan_closer_amount_inp = $( parentForm ).find('#loan_closer_amount');
			var payable_amount_inp = $( parentForm ).find('#payable_amount');

			var emi_amount_with_lic_inp = $( parentForm ).find('#emi_amount_with_lic');
			var loan_closer_amount_with_lic_inp = $( parentForm ).find('#loan_closer_amount_with_lic');
			...
			$(loan_closer_amount_inp).val(0);
			$(payable_amount_inp).val(0);

			$(emi_amount_with_lic_inp).val(0);
			$(loan_closer_amount_with_lic_inp).val(0);
			...
			if (loan_amount_val > 1) {
				...
				var loan_closer_amount = loan_amount_val + interest_amount;
				var payable_amount = ( loan_amount_val ) - process_fee_val;

				$(loan_closer_amount_inp).val(loan_closer_amount);
				...
				var loan_closer_amount_with_lic = loan_closer_amount + one_percent;

				$( loan_closer_amount_with_lic_inp ).val( loan_closer_amount_with_lic );
				...
				if (total_emi_count >= 1) {
					$(emi_amount_inp).val(Math.ceil(loan_closer_amount / total_emi_count));
				} else {
					$(emi_amount_inp).val(loan_closer_amount);
				}

				if (total_emi_count >= 1) {
					$(emi_amount_with_lic_inp).val(Math.ceil(loan_closer_amount_with_lic / total_emi_count));
				} else {
					$(emi_amount_with_lic_inp).val(loan_closer_amount_with_lic);
				}
